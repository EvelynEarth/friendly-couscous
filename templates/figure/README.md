...
| `figure_skill_evals.md` | 23个历史/对抗性回归测试：稀疏scatter、假CI、AI卡片、机制图文字/图标/布线、Draw.io XML、串问、advanced-chart漏用/滥用等 |
...
| `anti_ai_figure_gate.md` | 去 AI 信息图 / dashboard / PPT 味硬门；区分 Data Figure 与可少量使用语义图标的 Mechanism Figure |
...
当前包含 **23 个**对抗性案例。目标不是“文档看起来完整”，而是保证 Skill 在历史高频失败上做出正确判断。
